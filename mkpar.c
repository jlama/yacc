...
{
	parser = ecalloc(nstates, sizeof(*parser));
...
unused_rules(void)
{
	...
	rules_used = ecalloc(nrules, sizeof(Value_t));
...
remove_conflicts(void)
{
	...
	SRtotal = 0;
	RRtotal = 0;
	SRconflicts = ecalloc(nstates, sizeof(Value_t));
	RRconflicts = ecalloc(nstates, sizeof(Value_t));
	for (i = 0; i < nstates; i++) {
		SRcount = 0;
		RRcount = 0;
		symbol = -1;
...
defreds(void)
{
	defred = ecalloc(nstates, sizeof(Value_t));
	for (int i = 0; i < nstates; i++)
