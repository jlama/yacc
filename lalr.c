...
set_state_table(void)
{
	...
	state_table = ecalloc(nstates, sizeof(core *));
...
set_accessing_symbol(void)
{
	...
	accessing_symbol = ecalloc(nstates, sizeof(Value_t));
	for (sp = first_state; sp; sp = sp->next)
...
set_shift_table(void)
{
	...
	shift_table = ecalloc(nstates, sizeof(shifts *));
...
set_reduction_table(void)
{
	...
	reduction_table = ecalloc(nstates, sizeof(reductions *));
...
initialize_LA(void)
{
	...
	lookaheads = ecalloc(nstates + 1, sizeof(Value_t));
	...
	LA = ecalloc(k * tokensetsize, sizeof(unsigned));
	LAruleno = ecalloc(k, sizeof(Value_t));
	lookback = ecalloc(k, sizeof(shorts *));
...
set_goto_map(void)
{
	...
	goto_base = ecalloc(nvars + 1, sizeof(Value_t));
	temp_base = ecalloc(nvars + 1, sizeof(Value_t));
	...
	from_state = ecalloc(ngotos, sizeof(Value_t));
	to_state   = ecalloc(ngotos, sizeof(Value_t));
...
initialize_F(void)
{
	...
	F = ecalloc(nwords, sizeof(unsigned));

	reads = ecalloc(ngotos, sizeof(Value_t *));
	edge  = ecalloc(ngotos + 1, sizeof(Value_t));
	nedges = 0;
	...
	for (i = 0; i < ngotos; i++) {
		...
		if (sp) {
			...
			if (nedges) {
				reads[i] = rp = ecalloc(nedges + 1, sizeof(Value_t));
...
build_relations(void)
{
	...
	includes = ecalloc(ngotos, sizeof(Value_t *));
	edge = ecalloc(ngotos + 1, sizeof(*edge));
	states = ecalloc(maxrhs + 1, sizeof(*states));
	...
	for (i = 0; i < ngotos; i++) {
		...
		if (nedges) {
			includes[i] = shortp = ecalloc(nedges + 1, sizeof(Value_t));
			for (j = 0; j < nedges; j++)
				shortp[j] = edge[j];
			shortp[nedges] = -1;
		}
...
transpose(Value_t **R2, int n)
{
	...
	nedges = ecalloc(n, sizeof(*nedges));
	...
	new_R = ecalloc(n, sizeof(*new_R));
	temp_R = ecalloc(n, sizeof(*temp_R));

	for (i = 0; i < n; i++) {
		k = nedges[i];
		if (k > 0) {
			sp = ecalloc(k + 1, sizeof(*sp));
			new_R[i] = sp;
			temp_R[i] = sp;
			sp[k] = -1;
		}
...
digraph(Value_t **relation)
{
	...
	INDEX = ecalloc(ngotos + 1, sizeof(Value_t));
	VERTICES = ecalloc(ngotos + 1, sizeof(Value_t));
	top = 0;
