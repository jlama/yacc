...
cachec(int c)
{
	...
	if (cinc >= cache_size) {
		...
		cache = erealloc(cache, cache_size);
	}
	cache[cinc] = (char)c;
	++cinc;
}
...
#define UNLESS(what)        \
	...
	if (ld == ldOK) {
		...
		if (need > input_file_name_len) {
			...
			input_file_name = erealloc(input_file_name, input_file_name_len);
		}
		// XXX: that cast doesn't look right
		if ((long)need > 0) {
...
save_param(int k, char *buffer, int name, int type2)
{
	...
	p = emalloc(sizeof(*p));
	p->type2 = estrdup(buffer + type2);
	buffer[type2] = '\0';
	(void)trim_blanks(p->type2);

	p->name = estrdup(buffer + name);
	buffer[name] = '\0';
	(void)trim_blanks(p->name);

	p->type = estrdup(buffer);
...
cache_tag(char *tag, size_t len)
{
	...
	if (ntags >= tagmax) {
		tagmax += 16;
		tag_table = ereallocarray(tag_table, tagmax, sizeof(char *));
	}
...
declare_argtypes(bucket *bp)
{
	...
	bp->args = args;
	bp->argnames = emalloc(sizeof(char *) * args);
	bp->argtags = ecalloc(sizeof(char *), args + 1);
	while (--args >= 0) {
...
read_declarations(void)
{
	...
	for (;;) {
		...
		case POSIX_YACC:
			 ...
			 * byacc is already designed to be posix  yacc compatible.
			 */
			break;
		}
	}
...
initialize_grammar(void)
{
	...
	pitem = ecalloc(maxitems, sizeof(bucket *));

	nrules = 3;
	maxrules = 100;
	plhs = ecalloc(maxrules, sizeof(bucket *));
	rprec = ecalloc(maxrules, sizeof(Value_t));

	rassoc = ecalloc(maxrules, sizeof(Assoc_t));
	rassoc[0] = TOKEN;
	rassoc[1] = TOKEN;
	rassoc[2] = TOKEN;
}
...
{
	maxitems += 300;
	pitem = ereallocarray(pitem, maxitems, sizeof(bucket *));
}
...
expand_rules(void)
{
	...
	plhs = ereallocarray(plhs, maxrules, sizeof(bucket *));
	rprec = ereallocarray(rprec, maxrules, sizeof(Value_t));
	rassoc = ereallocarray(rassoc, maxrules, sizeof(Assoc_t));
}
...
parse_arginfo(bucket *a, char *args, int argslen)
{
	...
	if (a->args >= 0) {
		...
		if ((a->args = argslen) == 0)
			return;
		a->argnames = emalloc(sizeof(char *) * argslen);
		a->argtags = emalloc(sizeof(char *) * argslen);
	}
	if (!args)
		return;
	for (i = 0; i < argslen; i++) {
...
compile_arg(char **theptr, char *yyvaltag)
{
	...
	if (maxoffset > 0) {
		offsets = emalloc((maxoffset + 1) * sizeof(Value_t));
...
{
	struct arg_cache *entry = emalloc(sizeof(*entry));
	int i = strnshash(code) % ARG_CACHE_SIZE;
	entry->code = code;
	entry->rule = rule;
...
copy_destructor(void)
{
	...
	for (;;) {
		...
		if (c == EOF)
			unexpected_EOF();
		if (c == '<') {
			if (cptr[1] == '>') { /* "no semantic type" default destructor */
				cptr += 2;
				...
				if (bp->destructor != NULL)
					...
					bp->destructor = estrdup(code_text);
				}
			} else { /* "semantic type" default destructor */
				char *tag = get_tag();
		...
		} else if (isalpha(UCH(c)) || c == '_' || c == '.' || c == '$') { /* "symbol" destructor */
			bp = get_name();
			if (bp->destructor != NULL)
				destructor_redeclared_warning(&a);
			else {
...
pack_names(void)
{
	...
	name_pool = emalloc(name_pool_size);
...
pack_symbols(void)
{
	...
	symbol_name = emalloc(nsyms * sizeof(char *));
	symbol_value = emalloc(nsyms * sizeof(Value_t));
	symbol_prec = emalloc(nsyms * sizeof(Value_t));
	symbol_assoc = emalloc(nsyms);
	symbol_pval = emalloc(nsyms * sizeof(Value_t));

	if (destructor) {
		symbol_destructor = ecalloc(sizeof(char *), nsyms);
		symbol_type_tag = ecalloc(sizeof(char *), nsyms);
	}

	v = emalloc(nsyms * sizeof(bucket *));
	v[0] = 0;
	v[start_symbol] = 0;
	...
	if (gflag) {
		symbol_pname = emalloc(sizeof(char *) * nsyms);
...
pack_grammar(void)
{
	...
	ritem = ereallocarray(NULL, nitems, sizeof(Value_t));
	rlhs = ereallocarray(NULL, nrules, sizeof(Value_t));
	rrhs = ereallocarray(NULL, nrules + 1, sizeof(Value_t));
	rprec = ereallocarray(rprec, nrules, sizeof(Value_t));
	rassoc = ereallocarray(rassoc, nrules, sizeof(Assoc_t));
...
finalize_destructors(void)
{
	...
	for (i = 2; i < nsyms; ++i) {
		...
		if (symbol_destructor[i] == NULL) {
			if (tag == NULL) { /* use <> destructor, if there is one */
				if ((bp = default_destructor[UNTYPED_DEFAULT]) != NULL) {
					symbol_destructor[i] = estrdup(bp->destructor);
				}
			} else { /* use type destructor for this tag, if there is one */
				bp = lookup_type_destructor(tag);
				if (bp->destructor != NULL) {
					symbol_destructor[i] = estrdup(bp->destructor);
				} else { /* use <*> destructor, if there is one */
					if ((bp = default_destructor[TYPED_DEFAULT]) != NULL)
						/* replace "$$" with "(*val).tag" in destructor code */
						symbol_destructor[i]
		...
		} else { /* replace "$$" with "(*val)[.tag]" in destructor code */
			symbol_destructor[i]
