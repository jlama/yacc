...
allocate_itemsets(void)
{
	...
	count = 0;
	symbol_count = ecalloc(nsyms, sizeof(*symbol_count));
	...
	kernel_base = ecalloc(nsyms, sizeof(*kernel_base));
	kernel_items = ecalloc(count, sizeof(*kernel_items));
	...
	kernel_end = ecalloc(nsyms, sizeof(*kernel_end));
}
...
{
	allocate_itemsets();
	shiftset = ecalloc(nsyms, sizeof(*shiftset));
	redset = ecalloc(nrules + 1, sizeof(*redset));
	state_set = ecalloc(nitems, sizeof(*state_set));
}
...
{
	allocate_storage();
	itemset = ecalloc(nitems, sizeof(Value_t));
	ruleset = ecalloc(WORDSIZE(nrules), sizeof(unsigned));
	set_first_derives();
	initialize_states();
...
initialize_states(void)
{
	...
	p = emalloc(sizeof(core) + i * sizeof(Value_t));
	p->next = 0;
	p->link = 0;
	p->number = 0;
...
new_state(int symbol)
{
	...
	p = ecalloc(1, (sizeof(core) + (n - 1) * sizeof(Value_t)));
...
save_shifts(void)
{
	...
	p = ecalloc(1, (sizeof(shifts) + (unsigned)(nshifts - 1) * sizeof(Value_t)));
...
save_reductions(void)
{
	...
	if (count) {
		p = ecalloc(1, (sizeof(reductions) + (unsigned)(count - 1) * sizeof(Value_t)));
		p->number = this_state->number;
		p->nreds = count;
...
set_derives(void)
{
	...
	derives = ecalloc(nsyms, sizeof(Value_t *));
	rules = ecalloc(nvars + nrules, sizeof(Value_t));
