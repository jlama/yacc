...
output_accessing_symbols(void)
{
	...
	translate = emalloc(nstates * sizeof(*translate));
...
token_actions(void)
{
	...
	actionrow = ecalloc(PER_STATE * ntokens, sizeof(*actionrow));
	for (i = 0; i < nstates; ++i) {
		if (!parser[i])
			continue;
		...
		if (backtrack)
			width[2 * nstates + i] = 0;
		if (shiftcount > 0) {
			froms[i] = r = ecalloc(shiftcount, sizeof(Value_t));
			tos[i] = s = ecalloc(shiftcount, sizeof(Value_t));
			min = MAXYYINT;
			max = 0;
			for (j = 0; j < ntokens; ++j) {
				if (actionrow[j]) {
		...
		if (reducecount > 0) {
			froms[nstates + i] = r = ecalloc(reducecount, sizeof(Value_t));
			tos[nstates + i] = s = ecalloc(reducecount, sizeof(Value_t));
			min = MAXYYINT;
			max = 0;
			for (j = 0; j < ntokens; ++j) {
		...
		if (backtrack && conflictcount > 0) {
			froms[2 * nstates + i] = r = ecalloc(conflictcount, sizeof(Value_t));
			tos[2 * nstates + i] = s = ecalloc(conflictcount, sizeof(Value_t));
			min = MAXYYINT;
			max = 0;
			for (j = 0; j < ntokens; ++j) {
...
save_column(int symbol, int default_state)
{
	...
	froms[symno] = sp1 = sp = ecalloc(count, sizeof(Value_t));
	tos[symno] = sp2 = ecalloc(count, sizeof(Value_t));
...
goto_actions(void)
{
	...
	state_count = ecalloc(nstates, sizeof(Value_t));
...
sort_actions(void)
{
	...
	order = ecalloc(nvectors, sizeof(Value_t));
	nentries = 0;
	...
	for (;; ++j) {
		...
		for (k = 0; ok && k < t; k++) {
			...
			if (loc >= maxtable - 1) {
				...
				table = ereallocarray(table, newmax, sizeof(Value_t));
				check = ereallocarray(check, newmax, sizeof(Value_t));
...
pack_table(void)
{
	...
	base = ecalloc(nvectors, sizeof(Value_t));
	pos = ecalloc(nentries, sizeof(Value_t));

	maxtable = 1000;
	table = ecalloc(maxtable, sizeof(Value_t));
	check = ecalloc(maxtable, sizeof(Value_t));
...
output_actions(void)
{
	...
	froms = ecalloc(nvectors, sizeof(Value_t *));
	tos = ecalloc(nvectors, sizeof(Value_t *));
	tally = ecalloc(nvectors, sizeof(Value_t));
	width = ecalloc(nvectors, sizeof(Value_t));

	if (backtrack && (SRtotal + RRtotal) != 0)
		conflicts = ecalloc(4 * (SRtotal + RRtotal), sizeof(Value_t));
...
output_debug(void)
{
	...
	symnam = ecalloc(max + 2, sizeof(*symnam));
	for (i = nsyms - 1; i >= 0; --i)
